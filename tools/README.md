...
算法数学不在这些脚本里实现，全部 import `algo_reference/src/aim`。`common.py` 把该目录
...
注意：`firmware/include/config.h` 目前没有相机内参宏，固件的内参存在 NVS 里
（见 `firmware/src/calibration/calibration.h`）。脚本输出的 C 片段按 config.h 现有的
